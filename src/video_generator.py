...
def create_text_image(text: str, width: int, height: int, font_size: int = 5000, color: str = "white") -> Image.Image:
    """Pillow ile metin içeren bir görsel oluştur (çok büyük yazı)."""
...
def fix_resize_bug(clip, newsize):
    """ANTIALIAS hatasını LANCZOS ile düzelt."""
    import numpy as np
    ...
    def resize_frame(pic):
        if isinstance(pic, np.ndarray):
            ...
            resized_pil = pilim.resize(newsize[::-1], Image.LANCZOS)  # 👈 ANTIALIAS YERİNE LANCZOS
            return np.array(resized_pil)
        else:
            raise TypeError("MoviePy Error: resize_frame only works with numpy arrays")
...
def create_shorts_video(audio_path: str, script: str, output_path: str):
    ...
    if os.path.exists(sd_background_path):
        ...
        # ANTIALIAS HATASINI DÜZELT
        background = fix_resize_bug(background, (width, height)).set_duration(total_duration)
    else:
        # Yoksa siyah arka plan
        background = ColorClip(size=(width, height), color=(0, 0, 0), duration=total_duration)
    
    # 4. Metin görselleri oluştur (YAVAŞLA)
    text_clips = []
    start_time = 0.0
    for line in lines:
        ...
        # Satır süresi (daha YAVAŞ)
        word_count = len(line.split())
        duration = max(2.5, min(word_count * 0.75, total_duration - start_time))  # 👈 0.35 yerine 0.75
        end_time = start_time + duration
        
        # Pillow ile metin görseli oluştur (ÇOK BÜYÜK YAZI!)
        text_img = create_text_image(line, width, height, font_size=5000, color="white")  # 👈 5 kat artırıldı ✅
...
def create_podcast_video(audio_path: str, script: str, output_path: str):
    ...
    for line in lines:
        ...
        # Satır süresi (daha YAVAŞ)
        word_count = len(line.split())
        duration = max(4.0, min(word_count * 0.8, total_duration - start_time))  # 👈 0.4 yerine 0.8
        end_time = start_time + duration
