...
def generate_script(topic: str, mode: str = "shorts") -> str:
    ...
    if mode == "shorts":
        ...
        max_length = Config.SHORTS_CHAR_LIMIT  # 1000
    else:
        prompt = get_podcast_prompt(topic)
        max_length = Config.PODCAST_CHAR_LIMIT  # 15000
    ...
    # max_new_tokens yerine max_length kullan
    response = client.text_generation(
        prompt,
        max_length=max_length,  # 👈 max_length ile karakter sınırı
        truncation=True,        # 👈 Kesme uyarısını gider
        temperature=0.75,
        top_p=0.9,
    ...
    if len(script) > max_length:
        script = script[:max_length-50] + "... " + script[-50:]
    ...
    if not any(phrase in script.lower() for phrase in cta_phrases):
        ...
        if len(script) + len(cta) <= max_length:
            script += cta
