...
def authenticate_youtube():
    ...
    # token.pickle varsa yükle
    token_path = decode_token_pickle()
    creds = None
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # GitHub Actions'ta headless mod
            creds = flow.run_local_server(port=0, open_browser=False)
        
        # Yeni token.pickle oluştur (geçici klasörde)
        temp_token_path = Config.TEMP_DIR / "token.pickle"
